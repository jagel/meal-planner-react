import { ILanguageModel } from "../../common/models/lang.model";


export const LanguageData : ILanguageModel[] = [
    // ---------- A
    { code: 'agenda', value : 'Agenda' },
    { code: 'available days', value : 'Available Days' },

    // ---------- M
    { code: 'meal planner', value : 'Meal planner' },
    { code: 'meal planner agenda', value : 'Meal Planner Agenda' },
    { code: 'meal planner columns', value : 'Meal Planner Columns' },

    // ---------- S
    { code: 'settings', value : 'Settings' },
    
    { code: 'email', value : 'Email' },
    { code: 'password', value : 'Password' },
    { code: 'invalid email or password', value : 'Email or password invalid' },
    { code: 'login', value : 'Login' },
    
    { code: 'required field', value : 'Field is required' },
   
    { code: 'home', value : 'Home' },
    { code: 'recipes', value : 'Recipes' },
    { code: 'recipe', value : 'Recipe' },
    { code: 'update', value : 'Update' },
    { code: 'view', value : 'View' },
    { code: 'create', value : 'New' },
    { code: 'update', value : 'Update' },

    { code: 'Recipes', value : 'Recipes' },
    { code: 'Create Recipe', value : 'Create Recipe' },
    { code: 'Update Recipe', value : 'Update Recipe' },
    { code: 'View Recipe', value : 'View Recipe' },
    
    { code: 'directions', value:'Directions'},
    { code: 'add step', value:'Add Step'},
    { code: 'add ingredient', value:'Add Ingredient'},
    { code: 'ingredient name', value: 'Ingredient Name'},
    { code: 'step', value:'Step'},
    { code: 'steps', value:'Steps'},
    { code: 'step {0}', value:'Step {0}'},
    { code: 'measure type', value:'Measure Type'},
    { code: 'quantity', value:'Quantity'},

    { code: 'Home', value : 'Home' },
    { code: 'Search', value : 'Search' },
    { code: 'search recipe', value : 'Search Recipe' },
    { code: 'New Recipe', value : 'New Recipe' },
    { code: 'Manage', value : 'Manage' },

    { code: 'Cuisine', value : 'Cuisine' },
    { code: 'Name', value : 'Name' },
    { code: 'Recipe Name', value : 'Recipe Name' },
    { code: 'Recipe Search', value : 'Recipe Search' },
    { code: 'Recipe Description', value : 'Recipe Description' },
    { code: 'ingredients', value : 'Ingredients' },
    { code: 'units', value : 'Units' },
    { code: 'fraction', value : 'Fraction' },

    { code: 'name', value : 'Name' },
    { code: 'add', value : 'Add' },
    { code: 'continue', value : 'Continue' },
    
    { code: 'save', value:'Save' },
]