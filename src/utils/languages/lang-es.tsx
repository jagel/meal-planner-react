import { ILanguageModel } from "../../common/models/lang.model";


export const LanguageData : ILanguageModel[] = [
    // ---------- A
    { code: 'agenda', value : 'Agenda' },
    { code: 'available days', value : 'Días habiles' },

    // ---------- M
    { code: 'meal planner', value : 'Meal planner' },
    { code: 'meal planner agenda', value : 'Meal Planner Agenda' },
    { code: 'meal planner columns', value : 'Meal Planner Columnas' },
    { code: 'meal planner settings', value : 'Configuración Meal Planner' },
    
    // ---------- S
    { code: 'settings', value : 'Configuración' },

    { code: 'email', value : 'Correo Electrónico' },
    { code: 'password', value : 'Contraseña' },
    { code : 'invalid email or password', value : 'Verifique Correo Electrónico o Contraseña' },
    { code: 'login', value : 'Iniciar Sesión' },

    { code: 'required field', value : 'Campo requerido' },

    { code: 'home', value : 'Inicio' },
    { code: 'recipes', value : 'Recetas' },
    { code: 'recipe', value : 'Receta' },
    { code: 'update', value : 'Actualizar' },
    { code: 'view', value : 'Ver' },
    { code: 'create', value : 'Nuevo' },
    { code: 'update', value : 'Actualizar' },

    { code: 'Recipes', value : 'Recetas' },
    { code: 'Create Recipe', value : 'Nueva Receta' },
    { code: 'Update Recipe', value : 'Editar Receta' },
    { code: 'View Recipe', value : 'Ver Recea' },


    { code: 'directions', value:'Instrucciones'},
    { code: 'add step', value:'Agregar paso'},
    { code: 'add ingredient', value:'Agregar Ingrediente'},
    { code: 'ingredient name', value: 'Nombre del ingrediente'},
    { code: 'step', value:'Paso'},
    { code: 'steps', value:'Pasos'},
    { code: 'step {0}', value:'Paso {0}'},
    { code: 'measure type', value:'Unidad de medida'},
    { code: 'quantity', value:'Cantidad'},

    { code: 'Home', value : 'Inicio' },
    { code: 'Search', value : 'Busqueda' },
    { code: 'search recipe', value : 'Buscar Receta' },
    { code: 'New Recipe', value : 'Nueva Receta' },
    { code: 'Manage', value : 'Administrador' },

    { code: 'Cuisine', value : 'Cocina' },
    { code: 'Name', value : 'Nombre' },
    { code: 'Recipe Name', value : 'Nombre de la receta' },
    { code: 'Recipe Search', value : 'Buscar Receta' },
    { code: 'Recipe Description', value : 'Descripcion' },
    { code: 'ingredients', value : 'Ingredientes' },
    { code: 'units', value : 'Unidades' },
    { code: 'fraction', value : 'Fraccion' },

    { code: 'name', value : 'Nombre' },
    { code: 'add', value : 'Agregar' },
    { code: 'continue', value : 'Continuar' },

    { code: 'save', value:'Guardar' },
]